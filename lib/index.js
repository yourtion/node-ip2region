  ...
  constructor(options = {}) {
    ...
    if (this.headerSip == null) {
      ...
      for (let i = 0; i < this.totalHeaderLength; i += 8) {
        ...
        this.dataPtr = getLong(this.headerIndexBuffer, i + 4);
        if (this.dataPtr === 0) break;
              
        this.headerSip.push(startIp);
        this.headerPtr.push(this.dataPtr);
    ...
    while(low <= high) {
      ...
      if (ip < sip) {
        ...
        if (ip > eip) {
          ...
          this.dataPtr = getLong(blockBuffer, p + 8);
          break;
        }
      }
    }
      ...
      // read data
    if (this.dataPtr === 0) return null;
    const dataLen = ((this.dataPtr >> 24) & 0xFF);
    this.dataPtr = (this.dataPtr & 0x00FFFFFF);
    const dataBuffer = new Buffer(dataLen);

    fs.readSync(this.db, dataBuffer, 0, dataLen, this.dataPtr);
